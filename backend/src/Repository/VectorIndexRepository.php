<?php

namespace App\Repository;

use App\Entity\VectorIndex;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Persistence\ManagerRegistry;
use Sylius\Bundle\ResourceBundle\Doctrine\ORM\ResourceRepositoryTrait;
use Sylius\Resource\Doctrine\Persistence\RepositoryInterface as SyliusRepositoryInterface;

/**
 * @extends ServiceEntityRepository<VectorIndex>
 * @implements SyliusRepositoryInterface<VectorIndex>
 */
class VectorIndexRepository extends ServiceEntityRepository implements SyliusRepositoryInterface
{
    use ResourceRepositoryTrait;

    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, VectorIndex::class);
    }

    public function findOneByCollectionId(string $collectionId): ?VectorIndex
    {
        return $this->findOneBy(['collectionId' => $collectionId]);
    }

    /**
     * @return VectorIndex[]
     */
    public function findActive(): array
    {
        return $this->findBy(['isActive' => true]);
    }
}
